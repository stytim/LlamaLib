...
namespace UndreamAI.LlamaLib
{
    ...
    public class ChatMessage
    {
        ...
        /// <summary>
        /// Construct with string content (most common case)
        /// </summary>
        public ChatMessage(string _role, string _content)
        {
            role = _role;
            content = _content != null ? JToken.FromObject(_content) : JValue.CreateNull();
        }

        /// <summary>
        /// Construct with JToken content (for multimodal messages)
        /// </summary>
        public ChatMessage(string _role, JToken _content)
        {
            role = _role;
            content = _content ?? JValue.CreateNull();
        }

        /// <summary>
        /// Get content as plain string.
        ...
        public JObject ToJson()
        {
            ...
            {
                ["role"] = role,
                ["content"] = content ?? JValue.CreateNull()
            };
        }
        ...
        public static ChatMessage FromJson(JObject json)
        {
            return new ChatMessage(
                ...
                json["content"] ?? JValue.CreateNull()
            );
        }
        ...
        public override bool Equals(object obj)
        {
            if (obj is not ChatMessage other)
                return false;
            return role == other.role && JToken.DeepEquals(content, other.content);
        }
        ...
        public override int GetHashCode()
        {
            unchecked
            {
                ...
                hash = hash * 23 + (content?.ToString()?.GetHashCode() ?? 0);
                return hash;
            }
        }
    ...
    public class LLMAgent : LLMLocal
    {
        ...
        /// <summary>
        /// Add a user message with JSON content (for multimodal messages with images).
        /// Content should be a JSON array like: [{"type":"text","text":"..."},{"type":"image_url","image_url":{"url":"data:..."}}]
        /// </summary>
        public void AddUserMessage(JToken content)
        {
            CheckLlamaLib();
            if (content == null || content.Type == JTokenType.String)
            {
                llamaLib.LLMAgent_Add_User_Message(llm, content?.ToString() ?? string.Empty);
            }
            else
            {
                llamaLib.LLMAgent_Add_User_Message_Json(llm, content.ToString(Newtonsoft.Json.Formatting.None));
            }
        }
